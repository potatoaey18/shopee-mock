...
app.get('/', (req, res) => {
  ...
  const shippingCount  = DB.orders.filter(o => (o.order_status === 'SHIPPED' || o.order_status === 'PROCESSED') && (DB.deliveryStatus[o.order_sn] ?? 0) < 4).length;
    ...
    <table>
      ...
      <tbody id="orders-tbody">
        ${DB.orders.map(o => {
          ...
          const isShipped  = (o.order_status === 'SHIPPED' || o.order_status === 'PROCESSED');
...
async function handleDownloadShippingDocument(req, res) {
  ...
  const result_list = await Promise.all(order_list.map(async (item) => {
    ...
    try {
      ...
      return { order_sn, status: 'FAILED', file_type: 'PDF', file_data: '', fail_error: 'pdf_error', fail_message: err.message };
    }
  }));
